...
public class Case2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			switch (DEPLOYMENT) {
				case "EDGE":
					moduleMapping.addModuleToDevice("connector", "m-0-0");
					moduleMapping.addModuleToDevice("concentration_calculator", "m-0-0");
					moduleMapping.addModuleToDevice("client", "m-0-0");
					break;
				case "ROUTER":
					moduleMapping.addModuleToDevice("connector", "d-0");
					moduleMapping.addModuleToDevice("concentration_calculator", "d-0");
					moduleMapping.addModuleToDevice("client", "d-0");
					break;
				case "SERVER":
	...
	private static FogDevice addGw(String id, int userId, String appId, int parentId){
		...
		dept.setUplinkLatency(getLatencyValue()); // latency of connection between gateways and proxy server
		for(int i=0;i<numOfMobilesPerDept;i++){
			...
			mobile.setUplinkLatency(getLatencyValue()); // latency of connection between the smartphone and proxy server
	...
	private static FogDevice addMobile(String id, int userId, String appId, int parentId){
		...
		Sensor eegSensor = new Sensor("s-"+id, "EEG", userId, appId, new DeterministicDistribution(getWorkloadValue())); // inter-transmission time of EEG sensor follows a deterministic distribution
